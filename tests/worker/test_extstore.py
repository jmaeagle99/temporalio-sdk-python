...
class TestBaseDriver(Driver, WithSerializationContext):
...
class TestPayloadNotFoundDriver(TestBaseDriver):
    """Driver that stores normally but raises PayloadNotFoundError on retrieve."""

    def __init__(self, driver_name: str = "temporalio:driver:testnotfound"):
        super().__init__(driver_name, "temporalio:driver:testnotfound")
    ...
    async def retrieve(
        ...
        raise PayloadNotFoundError(
            "Payload not found in test driver",
            claim=claims[0] if claims else None,
            driver_name=self.name(),
        )


async def test_extstore_workflow_input_payload_not_found_fails_workflow(
    env: WorkflowEnvironment,
):
    ...
    client = await Client.connect(
        ...
        data_converter=dataclasses.replace(
            temporalio.converter.default(),
            external_storage=Options(
                drivers=[TestPayloadNotFoundDriver()],
                payload_size_threshold=1024,
            ),
        ),
    )
